...
\documentclass[notofandol]{mpltx}
...
% 可以使用 \zhdate 自动生成中文日期
\date{\zhdate{2022/5/25}, v2.0.1}
\begin{abstract}
    此部分为摘要.
...
\begin{table}
    \caption{表格示例.
        \note{注意使用 \texttt{ruledtabular} 环境.
            表格中的脚注会自动加在表尾.
            本表中的 ``数值'' 列使用了 \texttt{d} 列格式来按小数点位置对齐.
            为方便, 还提供了 \texttt{\textbackslash{}mc} 作为 \texttt{\textbackslash{}multicolumn} 的简写.
    ...
    \begin{ruledtabular}% ruledtabular 环境自动生成首尾双横线, 并调整宽度至占满全行
        \begin{tabular}{cd{4.2}lr}
            ...
            居中    & \mc{1}{c}{数值\footnote{记得将 d 列的列标题单独设为居中}} & 靠左    & 靠右 \\
            \colrule% 中间横线
            a     & 0.12                                      & a     & a \\
            bbb   & -100                                      & bbb   & bbb \\
